...
- every authorized node with stable `task_id`, role, user-request or named-pipeline source, provider, model, effort, inputs, outputs, and workspace policy;
...
For ordinary task sets, persist these role sources with `run-create --nodes-file` before `start --run-id`; see [node provenance and reporting](../scheduling-updates.md). A written manifest records a decision, not its authorization.
...
- For a pipeline, `checkpoint` is the liveness and recovery loop. Follow the [waiting and reporting rules](../../SKILL.md#waiting-and-reporting): use `--run-id` alone for a registered task set; otherwise select each task exactly once with either `--task-id` or `--starting-task-id` according to whether its record is confirmed present.
