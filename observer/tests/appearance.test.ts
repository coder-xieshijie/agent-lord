...
import {
  ...
  fontStack,
  readAppearance,
} from "../src/web/lib/appearance.js";

describe("appearance persistence", () => {
  it("migrates the old light/dark preference", () => {
    expect(
      readAppearance({
        getItem: (key) => (key === "observer-theme" ? "dark" : null),
      }),
    ).toEqual({ ...DEFAULT_APPEARANCE, theme: "dark" });
    expect(
      readAppearance({
        getItem: () => JSON.stringify({ theme: "nord", font: "Menlo" }),
      }),
    ).toEqual({ ...DEFAULT_APPEARANCE, theme: "nord", font: "Menlo" });
  });
