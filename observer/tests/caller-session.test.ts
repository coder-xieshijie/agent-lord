...
function fixture(
  ...
  if (options.indexLines)
    writeFileSync(
      path.join(root, "session_index.jsonl"),
      options.indexLines.map(line).join(""),
    );
  if (options.rollout !== false) {
    ...
    writeFileSync(
      ...
      line({
        type: "session_meta",
        payload: {
          id: options.metaId ?? "fixture-session",
          cwd: options.cwd ?? "/fixture/projects/fixture-project",
        },
      }),
    );
  }
...
function database(
  ...
  row: Partial<Record<TitleColumn, string | null>> & { id?: string },
  file = "state_5.sqlite",
  columns: TitleColumn[] = ["name", "preview", "title"],
) {
  const filename = path.join(caller.data_root!, file);
  const db = new DatabaseSync(filename);
  try {
    db.exec(
      `CREATE TABLE threads (id TEXT PRIMARY KEY, ${columns.map((column) => `${column} TEXT`).join(", ")})`,
    );
    db.prepare(
      `INSERT INTO threads (id, ${columns.join(", ")}) VALUES (?, ${columns.map(() => "?").join(", ")})`,
    ).run(
      row.id ?? caller.session_id!,
...
describe("caller session metadata (thread name + project)", () => {
  it("reads the latest index entry and the caller session's own project basename", () => {
    ...
    expect(reader.read(caller)).toEqual({
      name: "审查调度实现",
  ...
  it.each([
    ...
    const { caller, reader } = fixture({
      indexLines: [{ id: "fixture-session", thread_name: "索引名称" }],
    });
  ...
  it.each([
    ...
    "reads only the bound session in the newest database ($id)",
    ({ id, expected }) => {
      const { caller, reader } = fixture();
  ...
  it("refreshes a preview to a renamed title even when only the SQLite WAL changes", () => {
    ...
    try {
      ...
      writer
        .prepare("UPDATE threads SET name = ? WHERE id = ?")
        .run("重命名后的会话", caller.session_id!);
      expect(reader.read(caller).name).toBe("首条请求");
  ...
  it("falls back honestly when index or rollout metadata is missing", () => {
    ...
    const named = fixture({
      indexLines: [{ id: "fixture-session", thread_name: "只有名字" }],
      rollout: false,
    });
    expect(named.reader.read(named.caller)).toEqual({
      name: "只有名字",
      projectName: null,
    });
  });
  ...
  it("rejects unverifiable callers instead of guessing", () => {
    ...
    ).toEqual({ name: null, projectName: null });
    expect(
      reader.read({
        kind: "codex",
        session_id: "s",
  ...
  it("skips half-written index lines and tolerates an unreadable index", () => {
    const { caller, reader } = fixture({ rollout: false });
    writeFileSync(
      path.join(caller.data_root!, "session_index.jsonl"),
      `${JSON.stringify({ id: "fixture-session", thread_name: "完整行" })}\n{"id":"fixture-session","thread_na`,
    );
