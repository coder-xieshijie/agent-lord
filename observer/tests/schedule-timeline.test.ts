...
function op(
  operationId: string,
  extra: Partial<ScheduleOperation> = {},
): ScheduleOperation {
  return {
    operationId,
    kind: "turn",
    ...
    callerSessionId: "sess-main",
    dispatchTurnId: null,
    dispatchBinding: "none",
    createdAtMs: null,
    executorStartedAtMs: null,
    completedAtMs: null,
    providerCompletedAtMs: null,
    artifactExportedAtMs: null,
    artifactReadyApprox: false,
    hasArtifact: false,
    receipt: null,
    deliveryStatus: null,
    attempt: null,
...
const opA = op("op-a", {
  ...
  dispatchBinding: "recorded",
  createdAtMs: at(10),
  executorStartedAtMs: at(12),
  completedAtMs: at(200),
  hasArtifact: true,
  artifactExportedAtMs: at(199),
  receipt: { turnId: "turn-2", atMs: at(210), status: "SUCCEEDED" },
});
const opB = op("op-b", {
  dispatchTurnId: "turn-2",
  dispatchBinding: "inferred-by-create-time",
  createdAtMs: at(110),
  status: "running",
  running: true,
});
const opNoTime = op("op-ghost", {
  createdAtMs: null,
  status: "running",
  running: true,
});
...
describe("layoutSchedule", () => {
  it("lays out turns and lanes with honest open ends, clipping and in-range markers only", () => {
    ...
    expect(layout.turnSegments[1]).toMatchObject({
      open: true,
      clippedEnd: false,
    });
    // Lane order preserved; the no-evidence op gets no segment.
    expect(layout.lanes.map((lane) => lane.task.taskId)).toEqual([
      "task-a",
      "task-b",
    ]);
    expect(
      layout.lanes[1].segments.map((segment) => segment.operationId),
    ).toEqual(["op-b"]);
    const segmentA = layout.lanes[0].segments[0];
    expect(segmentA.openEnd).toBe(false);
    expect(segmentA.markers.map((marker) => marker.kind)).toEqual([
      "dispatch",
      "executor-start",
      "completed",
      "artifact",
    ...
    expect(clipped.lanes.map((lane) => lane.task.taskId)).toEqual([
      "task-a",
      "task-b",
    ]);
    expect(
      clipped.lanes[0].segments[0].markers.map((marker) => marker.kind),
    ...
    const lateA = later.lanes.find((lane) => lane.task.taskId === "task-a")!
      .segments[0];
  ...
  it("flags approximate artifact times instead of hiding the approximation", () => {
    const approx = op("op-approx", {
      createdAtMs: at(0),
      completedAtMs: at(5),
      hasArtifact: true,
      artifactExportedAtMs: null,
      artifactReadyApprox: true,
    });
    const g: ScheduleGroup = {
      ...group,
      turns: [],
      tasks: [task("task-x", [approx])],
    };
    const range = rangeOptions(g, NOW).find(
      (option) => option.key === "session",
    )!;
    const marker = layoutSchedule(
      g,
      range,
      NOW,
    ).lanes[0].segments[0].markers.find((item) => item.kind === "artifact")!;
...
describe("edgesForSelection", () => {
  it("draws dispatch and cross-turn receipt edges only for the selection", () => {
    const session = rangeOptions(group, NOW).find(
      (option) => option.key === "session",
    )!;
    const layout = layoutSchedule(group, session, NOW);
    expect(edgesForSelection(layout, null)).toEqual([]);
    const edges = edgesForSelection(layout, {
      taskId: "task-a",
      operationId: "op-a",
    });
    ...
    expect(edges[1].mainFrac).toBeCloseTo(
      (at(210) - session.startMs) / (session.endMs - session.startMs),
      6,
    );
    // Turn-1 range: the receipt arrived after the range → flagged out-of-range.
    const turn1 = rangeOptions(group, NOW).find(
      (option) => option.key === "turn:turn-1",
    )!;
    const clippedEdges = edgesForSelection(layoutSchedule(group, turn1, NOW), {
      taskId: "task-a",
      operationId: "op-a",
    });
    expect(clippedEdges.find((edge) => edge.kind === "receipt")).toMatchObject({
      outOfRange: true,
    });
    expect(
      edgesForSelection(layout, { taskId: "task-b", operationId: "op-ghost" }),
    ).toEqual([]);
  });
});
