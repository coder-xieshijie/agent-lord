...
function fixture(kind: "mcode" | "claude") {
  const root = mkdtempSync(path.join(tmpdir(), "caller-native-"));
  roots.push(root);
  ...
  return {
    root,
    caller,
    names: new CallerSessionReader(),
    lifecycle: new CallerLifecycleReader(),
  };
}
function mcode() {
  const f = fixture("mcode");
  const dir = path.join(f.root, "v2", "sqlite");
  mkdirSync(dir, { recursive: true });
  const file = path.join(dir, "runtime-state.sqlite");
  const db = new DatabaseSync(file);
  db.exec(
    "CREATE TABLE local_runtime_sessions(session_id TEXT PRIMARY KEY, title TEXT, workspace_dir TEXT); CREATE TABLE local_runtime_turn_ingress(session_id TEXT, turn_id TEXT, status TEXT, accepted_at_ms INTEGER, completed_at_ms INTEGER)",
  );
  ...
  db.prepare(
    "INSERT INTO local_runtime_turn_ingress VALUES (?, ?, ?, ?, ?)",
  ).run("session-1", "turn-1", "accepted", start, null);
  db.close();
  return { ...f, file };
}
function claude() {
  const f = fixture("claude");
  const dir = path.join(f.root, "projects", "project-a");
  mkdirSync(dir, { recursive: true });
  const file = path.join(dir, "session-1.jsonl");
  writeFileSync(file, "");
  const event = (value: Record<string, unknown>, seconds = 0) =>
    `${JSON.stringify({ sessionId: "session-1", timestamp: new Date(start + seconds * 1000).toISOString(), ...value })}\n`;
  appendFileSync(
    file,
    event({
      type: "user",
...
describe("native scheduling sessions", () => {
  it.each(["mcode", "claude"])(
    "projects a %s scheduler into the actual observer overview, snapshot and schedule",
    (kind) => {
      const f = kind === "mcode" ? mcode() : claude();
      mkdirSync(path.join(f.root, "operations"));
      writeFileSync(
        path.join(f.root, "operations", "op.json"),
        JSON.stringify({
          operation_id: "op",
          task_id: "child",
          provider: "mcode-cli",
          ...
          invocation: { caller: f.caller },
        }),
      );
      const hub = new Hub(["child"], f.root);
      hub.refresh();
      const meta = hub.overview()[0];
      expect(meta.caller?.session).toMatchObject({
        kind,
        sessionId: "session-1",
        projectName: "project-a",
      });
      expect(meta.caller?.lifecycle).toMatchObject({ status: "running" });
      expect(hub.snapshot("child")!.task.caller?.session).toEqual(
        meta.caller?.session,
      );
  ...
  it("reads MCode metadata and refreshes WAL-only turn completion without modifying the database", () => {
    ...
    expect(f.names.read(f.caller)).toEqual({
      name: "MCode 调度",
      projectName: "project-a",
    });
    expect(
    ...
    db.prepare(
      "UPDATE local_runtime_turn_ingress SET status = 'completed', completed_at_ms = ? WHERE session_id = ?",
    ).run(start + 10_000, "session-1");
  ...
  it("never creates a missing MCode DB, guesses another session, or accepts an unrelated turn", () => {
    ...
    const f = mcode();
    expect(f.names.read({ ...f.caller, session_id: "missing" })).toEqual({
      name: null,
      projectName: null,
    });
    expect(
      f.lifecycle.observe(
        { ...f.caller, turn_id: "unrelated" },
        "op",
        new Date(start + 1000).toISOString(),
        null,
      ).status,
    ...
    expect(
      f.lifecycle.sessionTimeline({ ...f.caller, session_id: "../../bad" })
        .turns,
    ).toEqual([]);
  });
  ...
  it("reads Claude names, ignores forked history/sidechains, and uses client end events rather than model stops", () => {
    ...
    appendFileSync(
      f.file,
      f.event({ type: "custom-title", customTitle: "Claude 调度" }),
    );
    appendFileSync(
      f.file,
      f.event({
        type: "custom-title",
        customTitle: "PRIVATE_OTHER",
        sessionId: "other",
      }),
    );
    appendFileSync(
    ...
    expect(f.lifecycle.observe(f.caller, "op", at, null).status).toBe(
      "running",
    );
    appendFileSync(
      f.file,
      f.event({ type: "assistant", message: { stop_reason: "end_turn" } }, 3),
    );
    ...
    appendFileSync(
      f.file,
      f.event({ type: "system", subtype: "turn_duration" }, 4),
    );
  ...
  it("indexes only actual Claude tool-result receipts and handles partial appends and truncation", () => {
          ...
          content: [
            {
              type: "tool_result",
              content: JSON.stringify({
                actionable: [{ status: "SUCCEEDED", operation_id: "op" }],
              }),
            },
          ],
        },
    ...
    expect(timeline.receipts.get("op")).toEqual({
      turnId: "user-1",
      atMs: start + 2000,
      status: "SUCCEEDED",
    });
    ...
    writeFileSync(
      f.file,
      f.event({ type: "user", uuid: "wrong", sessionId: "other" }),
    );
    now.mockReturnValue(start + 5000);
