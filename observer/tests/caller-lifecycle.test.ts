...
function fixture() {
  const root = mkdtempSync(path.join(tmpdir(), "fixture-caller-"));
  roots.push(root);
  ...
  writeFileSync(
    file,
    line("session_meta", { id: caller.session_id }) +
...
describe("bound Codex caller lifecycle", () => {
  it("keeps execution and caller completion separate and only exposes matching turn metadata", () => {
                    ...
                    actionable: [
                      {
                        ...
                        response: { text: "PRIVATE_FINAL_TEXT" },
                      },
                    ],
                  }),
                },
    ...
    expect(
      reader.observe(caller, "fixture-operation", created, ready),
    ).toMatchObject({
      status: "running",
      receivedAtMs: Date.parse(received),
      completedAtMs: null,
    });
    appendFileSync(
      file,
    ...
    expect(result).toMatchObject({
      status: "completed",
      turnId: "fixture-turn",
      completedAtMs: Date.parse("2026-09-09T04:33:32Z"),
    });
    expect(JSON.stringify(result)).not.toContain("PRIVATE");
    expect(
      reader.observe(
        { ...caller, turn_id: "later-turn" },
        "fixture-operation",
        created,
        ready,
      ).status,
  ...
  it("sessionTimeline exposes every verified turn and cross-turn multi-status receipts", () => {
    ...
    appendFileSync(
      ...
      line(
        "response_item",
        {
          ...
          output: JSON.stringify({
            actionable: [
              { status: "SUCCEEDED", operation_id: "op-cross" },
              { status: "ERROR", operation_id: "op-broken" },
            ],
          }),
        },
        "2026-09-09T04:32:00Z",
      ),
    );
    ...
    expect(timeline.turns).toEqual([
      {
        turnId: "fixture-turn",
        startedAtMs: Date.parse("2026-09-09T04:20:00Z"),
        completedAtMs: Date.parse("2026-09-09T04:30:00Z"),
        aborted: false,
      },
      {
        turnId: "turn-2",
        startedAtMs: Date.parse("2026-09-09T04:31:00Z"),
        completedAtMs: null,
        aborted: false,
      },
    ]);
    // Receipts land on the turn active at arrival (cross-turn reception).
    expect(timeline.receipts.get("op-cross")).toEqual({
      turnId: "turn-2",
      atMs: Date.parse("2026-09-09T04:32:00Z"),
      status: "SUCCEEDED",
    });
    expect(timeline.receipts.get("op-broken")).toEqual({
      turnId: "turn-2",
      atMs: Date.parse("2026-09-09T04:32:00Z"),
      status: "ERROR",
    });
    ...
    expect(
      reader.observe(caller, "fixture-operation", created, null).status,
    ).toBe("completed");
  });
  ...
  it("sessionTimeline reports an honest note instead of fabricated turns when the session is unverifiable", () => {
    ...
    const missing = reader.sessionTimeline({
      ...caller,
  ...
  it("never fabricates a finished state from a later turn or an unrelated completion", () => {
    ...
    appendFileSync(
      ...
      line(
        "event_msg",
        { type: "task_complete", turn_id: "unrelated" },
        "2026-09-09T04:30:00Z",
      ),
    );
    expect(
    ...
    appendFileSync(
      ...
      line(
        "event_msg",
        { type: "task_started", turn_id: "later" },
        "2026-09-09T04:40:00Z",
      ),
    );
    expect(
  ...
  it("checks session identity, tolerates partial appends and resets truncated files", () => {
    ...
    const terminal = line(
      "event_msg",
      { type: "task_aborted", turn_id: "fixture-turn" },
      "2026-09-09T04:30:00Z",
    );
    ...
    expect(
      reader.observe(undefined, "fixture-operation", created, null).status,
    ).toBe("unknown");
  });
});
