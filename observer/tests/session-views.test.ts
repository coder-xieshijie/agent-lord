...
function task(
  ...
  session: CallerSessionMeta | null | undefined = undefined,
): TaskMeta {
  return {
    taskId,
    title: taskId,
...
): CallerSessionMeta => ({ sessionId, name, projectName });
...
describe("sortTasksByActivity", () => {
  it("orders newest first, sinks missing timestamps and stays deterministic on ties", () => {
    ...
    expect(sortTasksByActivity(tasks).map((item) => item.taskId)).toEqual([
      "c-new",
      "a-tie",
      "d-tie",
      "b-old",
...
describe("groupTasksByCaller", () => {
  it("groups by scheduling session, orders groups and children newest first and keeps unattributed history reachable", () => {
    ...
    expect(groups[2].tasks.map((item) => item.taskId)).toEqual([
      "legacy",
      "no-caller-field",
    ]);
  });
  ...
  it("sorts all-missing-activity groups last but keeps them accessible", () => {
    const groups = groupTasksByCaller([
      task("no-time", null, session("sess-quiet")),
      task("active", 100, session("sess-live")),
    ]);
    expect(groups.map((group) => group.key)).toEqual([
      "caller:sess-live",
      "caller:sess-quiet",
...
it("keeps matching ids from different scheduling clients in distinct groups", () => {
  const a = task("mcode-task", 100, {
    ...session("same-id", "MCode"),
    kind: "mcode",
  });
  const b = task("claude-task", 200, {
    ...session("same-id", "Claude"),
    kind: "claude",
  });
