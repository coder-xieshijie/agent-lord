...
const tool = (
  id: string,
  state: ToolItem["state"] = "completed",
  opId: string | undefined = "fixture-op",
): ToolItem => ({
  id,
  kind: "tool",
  name: "read",
...
describe("reader-facing timeline", () => {
  ...
  it("never groups across messages, rounds, hidden lifecycle events or unknown operation identity", () => {
    const items: TimelineItem[] = [
      ...
      tool("c"),
      tool("d"),
      tool("e", "completed", "other-op"),
      { id: "round", kind: "lifecycle", name: "start", label: "开始", ord: 0 },
      tool("f", "completed", "other-op"),
      tool("g", "completed", "other-op"),
      { ...tool("h"), opId: undefined },
      { ...tool("i"), opId: undefined },
      { ...tool("j"), opId: undefined },
    ];
    expect(
      presentTimeline(items).rows.some((row) => row.kind === "tool-group"),
    ).toBe(false);
    expect(presentTimeline(items).details.map((row) => row.id)).toEqual([
      "round",
    ]);
  });
  ...
  it("keeps running tools, failures and history warnings visible", () => {
    ...
    expect(presentTimeline(items).rows.map((row) => row.id)).toEqual([
      "group:a",
      "active",
      "failed",
      "notice",
    ...
    expect(
      toolSummary([
        tool("a"),
        tool("b"),
        { ...tool("c"), name: "custom_tool" },
      ]),
    ).toBe("read ×2 · custom_tool");
  });
});
