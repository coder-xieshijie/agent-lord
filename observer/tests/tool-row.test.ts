...
const render = (item: Partial<ToolItem>) =>
...
describe("tool timing presentation", () => {
  ...
  it("shows preparation separately from execution", () => {
    ...
    const html = render({
      state: "completed",
      phase: "completed",
      preparationMs: 225000,
      executionMs: 53,
      errorText: null,
    });
    expect(html).toContain("参数准备 225.00s");
    expect(html).toContain("执行 0.05s");
    expect(html).not.toContain("执行失败");
  });
  it("does not invent unavailable timing", () => {
    expect(render({ state: "completed" })).not.toContain("参数准备");
  });
});
