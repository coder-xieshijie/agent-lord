...
import {
  ...
  callerStatus,
  elapsed,
} from "../src/web/components/execution-details.js";
...
describe("request and execution presentation", () => {
  it("preserves full escaped request text and labels an automatic follow-up with its actual caller", () => {
    const item: RequestItem = {
      ...
      kind: "request",
      role: "caller",
      text: "<script>not executed</script>" + "完整请求".repeat(400),
      trigger: "caller_followup",
      reason: "补齐原始需求",
      caller: {
        kind: "codex",
        session_id: "fixture-caller",
        turn_id: null,
    ...
    expect(
      renderToStaticMarkup(
        createElement(RequestRow, { item: { ...item, role: "user" } }),
      ),
    ).toContain("用户原始请求");
  });
  ...
  it("displays independent MCode effort and variants, complete ids and explicit missing metadata", () => {
    const meta = {
      provider: "mcode-cli",
      execution: {
        requestedModel: "provider/fable",
        actualModel: "provider/fable",
      ...
      caller: {
        initial: {
          session_id: "fixture-original-caller-full-id",
          identity_source: "caller-declared",
        },
        current: {
          kind: "codex",
          session_id: "fixture-current-caller-full-id",
          identity_source: "runtime-env",
        },
        lifecycle: { status: "unknown", turnId: null, note: "未找到记录" },
      },
    } as TaskMeta;
    const html = renderToStaticMarkup(
      createElement(ExecutionDetails, { meta }),
    );
    for (const text of [
      "请求模型",
      "实际模型",
      ...
      "thinking",
      "由启动参数约束",
      "fixture-original-caller-full-id",
      "fixture-current-caller-full-id",
      "主调度状态未知",
      "未观测",
    ])
    ...
    const timed = renderToStaticMarkup(
      createElement(ExecutionDetails, {
        meta: {
          ...meta,
          timing: {
            providerCompletedAtMs: 1788928362794,
            artifactReadyAtMs: null,
            callerReceivedAtMs: null,
            callerCompletedAtMs: null,
          },
        },
      }),
    );
  ...
  it("honors focused URLs, keeps manual selection and never silently substitutes a missing target", () => {
    const tasks = [
      { taskId: "fixture-a", running: true },
      { taskId: "fixture-b", running: false },
    ] as TaskMeta[];
    expect(selectTask(tasks, null, "?task=fixture-b&token=fixture")).toBe(
      "fixture-b",
    );
