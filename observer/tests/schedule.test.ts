...
} from "../src/server/schedule.js";
import type {
  CallerLifecycleReader,
  SessionTimeline,
} from "../src/server/caller-lifecycle.js";
...
const T = (offsetSec: number): string =>
  new Date(Date.UTC(2026, 8, 9, 4, 0, offsetSec)).toISOString();
...
function input(
  ...
  opTimes: ScheduleTaskInput["opTimes"] = new Map(),
): ScheduleTaskInput {
  return { meta: meta(taskId), operations, opTimes };
}

function stubLifecycle(
  timelines: Record<string, SessionTimeline>,
): CallerLifecycleReader {
  return {
...
const stubSessions = {
  read: () => ({ name: "调度会话名", projectName: "fixture-project" }),
} as unknown as CallerSessionReader;
...
describe("buildSchedule", () => {
  it("keeps identical session ids from different CLI clients separate", () => {
    const result = buildSchedule({
      ...
      tasks: ["mcode", "claude", "codex"].map((kind) =>
        input(kind, [
          op(kind, `op-${kind}`, {
            invocation: {
              caller: { ...caller("same-session"), kind },
    ...
    expect(new Set(result.groups.map((group) => group.kind))).toEqual(
      new Set(["mcode", "claude", "codex"]),
    );
  ...
  it("attributes by the first operation, keeps lanes stable, binds turns honestly and joins cross-turn receipts", () => {
    ...
    const result = buildSchedule({
      ...
      tasks: [
        ...
        input("task-a", [
          op("task-a", "op-a1", {
            invocation: {
              caller: caller("sess-main", "turn-1"),
              trigger: "user_request",
              user_request: null,
              reason: null,
            },
          ...
          [
            ...
            op("task-b", "op-b2", {
              ...
              parallel: {
                group: "grp",
                role: "integrator",
                integratorTaskId: null,
                workers: ["task-a"],
              },
            }),
          ],
          new Map([
            ["op-b2", { startedAtMs: Tms(111), artifactExportedAtMs: null }],
          ]),
        ),
        ...
        input("task-c", [
          op("task-c", "op-c1", {
            ...
            createdAt: T(0),
            completedAt: new Date(Tms(0) - 5000).toISOString(),
            status: "failed",
            errorCode: "E_FAIL",
          }),
          op("task-c", "op-c2", {
            invocation: {
              caller: caller("sess-other"),
              trigger: "caller_followup",
              user_request: null,
              reason: null,
            },
    ...
    expect(result.groups.map((group) => group.sessionId)).toEqual([
      "sess-main",
      null,
    ]);
    ...
    expect(main.tasks.map((task) => task.taskId)).toEqual([
      "task-c",
      "task-a",
      "task-b",
    ]);
    ...
    expect(b2).toMatchObject({
      ...
      running: true,
      completedAtMs: null,
      executorStartedAtMs: Tms(111),
      attempt: 1,
    });
    // Latest recorded parallel plan wins.
    expect(main.tasks[2].parallel).toEqual({
      group: "grp",
      role: "integrator",
      integratorTaskId: null,
      workers: ["task-a"],
    ...
    expect(c2).toMatchObject({
      callerSessionId: "sess-other",
      receipt: { status: "ERROR", turnId: "turn-2" },
    });
    ...
    expect(unattributed.tasks[0].operations[0]).toMatchObject({
...
describe("Hub schedule integration (journal evidence + restart replay)", () => {
  ...
  function fixtureRoot(): string {
    ...
    roots.push(root);
    for (const dir of ["operations", "logs", "events"])
  ...
  it("aggregates every operation with journal timestamps and rebuilds them after a restart", () => {
    ...
    expect(ops.map((operation) => operation.operationId)).toEqual([
      "fixture-op-1",
      "fixture-op-2",
    ]);
    expect(ops[0]).toMatchObject({
      executorStartedAtMs: Tms(2),
      artifactExportedAtMs: Tms(19),
    });
    expect(ops[1]).toMatchObject({
      executorStartedAtMs: Tms(31),
      completedAtMs: null,
      running: true,
    });
