  ...
  it("upgrades to the task record once it appears, keeping items stable", () => {
    ...
    writeFileSync(
      ...
      j({
        task_id: taskId,
        provider: "codex-cli",
        endpoint_id: "thread_fixture",
        contract: { model: "gpt-fixture" },
      }),
    );
...
describe("Hub ordering and incremental refresh", () => {
  it("projects the initial caller's Desktop preview into overview and snapshot without an index", () => {
    ...
    try {
      db.exec(
        "CREATE TABLE threads (id TEXT PRIMARY KEY, name TEXT, preview TEXT, title TEXT)",
      );
    ...
    for (const [index, session] of [
      "fixture-initial-caller",
      "fixture-current-caller",
    ].entries()) {
    ...
    const expected = {
      kind: "codex",
      sessionId: "fixture-initial-caller",
      name: "规划调度时间线",
      projectName: null,
    };
  ...
  it("replays two complete request/response rounds in order and keeps original and current callers separate", () => {
    ...
    const write = (
      ...
      const stdout = writeOperation(root, taskId, id, "mcode-cli", {
        status: "succeeded",
        created_at: `2026-09-09T${hour}:00:00Z`,
        completed_at: `2026-09-09T${hour}:01:00Z`,
        message: text,
        ...
        invocation: {
          caller: {
            ...
            data_root: "/private/not-for-web",
          },
          trigger: user_request ? "user_request" : "caller_followup",
          user_request,
          reason: user_request ? null : "旧 deny 处理缺失",
        },
      });
      appendFileSync(
        stdout,
        j({
          type: "item.completed",
          item: { id: "reply", type: "agent_message", content: `reply-${id}` },
        }) + "\n",
      );
    };
    write(
    ...
    expect(
      snapshot.items
        .filter((item) => item.kind === "request" || item.kind === "message")
    ...
    // no data_root leaks.
    expect(snapshot.task.caller?.session).toEqual({
      kind: "codex",
      sessionId: "caller-first",
      name: null,
      projectName: null,
    });
    expect(snapshot.task.execution).toMatchObject({
      requestedModel: "test/fable",
      actualModel: "test/fable",
    ...
    const replay = new Hub([taskId], root);
    replay.refresh();
  ...
  it("never infers the original launcher from later operations when the first one recorded no caller", () => {
    ...
    writeOperation(root, taskId, "fixture-late-first", "mcode-cli", {
  ...
  it("shows a Claude fallback as the actual model and does not treat argument-only Codex metadata as a runtime report", () => {
    ...
    writeOperation(
      ...
      "fixture-claude-model-op",
      "claude-cli",
      {
        expected: { model: "claude-fable-5", effort: "xhigh" },
        observed: {
    ...
    writeOperation(
      root,
      "fixture-codex-model",
      "fixture-codex-model-op",
      "codex-cli",
      {
        expected: { model: "gpt-fixture", effort: "high" },
        observed: {
          models: ["gpt-fixture"],
          model_verification: "argument-enforced",
        },
        message: "legacy dispatch",
      },
    );
    const hub = new Hub(["fixture-claude-model", "fixture-codex-model"], root);
    hub.refresh();
    expect(hub.snapshot("fixture-claude-model")!.task.execution).toMatchObject({
      requestedModel: "claude-fable-5",
      actualModel: "claude-opus-5",
      modelVerification: "provider-metadata",
    });
    ...
    expect(old.items.find((item) => item.kind === "request")).toMatchObject({
      text: "legacy dispatch",
      role: "caller",
  ...
  it("separates execution success, delivery evidence and recovery activity", () => {
    ...
    writeOperation(root, taskId, "fixture-evidence-op", "mcode-cli", {
      ...
      delivery: {
        ...
        checks: [{ kind: "file", path: "LICENSE", ok: false }],
      },
    });
    const hub = new Hub([taskId], root);
    hub.refresh();
  ...
  it("orders journal pre → stream → journal post within a cycle", () => {
    ...
    const stdout = writeOperation(root, taskId, "fixture-op-3", "codex-cli", {
      status: "succeeded",
    });
    appendFileSync(
      stdout,
      `${j({ type: "item.completed", item: { id: "item_0", type: "agent_message", text: "正文" } })}\n`,
    );
    writeFileSync(
    ...
    const kindsInOrder = hub
      .snapshot(taskId)!
      .items.map((item) =>
        item.kind === "journal" ? (item as { name: string }).name : item.kind,
      );
    expect(kindsInOrder.indexOf("operation-created")).toBeLessThan(
      kindsInOrder.indexOf("message"),
    );
    expect(kindsInOrder.indexOf("message")).toBeLessThan(
      kindsInOrder.indexOf("operation-succeeded"),
    );
  });
  ...
  it("picks up a new operation on the same task and half lines wait", () => {
    ...
    const stdout1 = writeOperation(root, taskId, "fixture-op-4", "codex-cli", {
      created_at: "2026-09-08T10:00:00Z",
    });
    appendFileSync(
      stdout1,
      `${j({ type: "item.completed", item: { id: "item_0", type: "agent_message", text: "第一轮" } })}\n`,
    );
    const hub = new Hub([taskId], root);
    hub.refresh();
    expect(
      hub.snapshot(taskId)!.items.filter((item) => item.kind === "message"),
    ).toHaveLength(1);

    const stdout2 = writeOperation(root, taskId, "fixture-op-5", "codex-cli", {
    ...
    appendFileSync(
      ...
      j({
        type: "item.completed",
        item: { id: "item_0", type: "agent_message", text: "第二轮" },
      }),
    ); // no trailing \n yet
    hub.refresh();
    expect(
      hub.snapshot(taskId)!.items.filter((item) => item.kind === "message"),
    ).toHaveLength(1); // half line not consumed
  ...
  it("notices an attempt switch and reads the new stream from the start", () => {
    ...
    appendFileSync(
      stdout1,
      `${j({ type: "assistant", message: { id: "m1", content: [{ type: "text", text: "尝试一" }] } })}\n`,
    );
    const hub = new Hub([taskId], root);
    hub.refresh();
    ...
    writeFileSync(
      attempt,
      `${j({ type: "assistant", message: { id: "m2", content: [{ type: "text", text: "尝试二" }] } })}\n`,
    );
    writeFileSync(
    ...
    expect(
      items.some(
        (item) =>
          item.kind === "notice" &&
          (item as { text: string }).text.includes("重试"),
      ),
    ).toBe(true);
    const texts = items
...
describe("Hub cursor + listeners", () => {
  it("serves deltas per cursor and rejects foreign-generation cursors", () => {
    ...
    appendFileSync(
      stdout,
      `${j({ type: "item.completed", item: { id: "item_0", type: "agent_message", text: "a" } })}\n`,
    );
    const hub = new Hub([taskId], root);
    hub.refresh();
    const snapshot = hub.snapshot(taskId)!;
    ...
    appendFileSync(
      stdout,
      `${j({ type: "item.completed", item: { id: "item_1", type: "agent_message", text: "b" } })}\n`,
    );
    hub.refresh();
  ...
  it("broadcasts new patches to every subscribed listener independently", () => {
    ...
    appendFileSync(
      stdout,
      `${j({ type: "item.completed", item: { id: "item_0", type: "agent_message", text: "推送" } })}\n`,
    );
    hub.refresh();
...
describe("Read-only access scope", () => {
  it("never accepts stdout paths outside <root>/logs or foreign prefixes", () => {
    ...
    expect(
      validStdoutPath(root, "op-1", path.join(root, "logs", "other-op.stdout")),
    ).toBeNull();
    expect(
      validStdoutPath(
        root,
        "op-1",
        path.join(root, "logs", "sub", "op-1.stdout"),
      ),
    ).toBeNull();
    expect(
      validStdoutPath(root, "op-1", path.join(root, "logs", "op-1.stdout")),
    ).toBe(path.join(root, "logs", "op-1.stdout"));
  });
  ...
  it("only exposes allow-listed tasks", () => {
    ...
    expect(hub.overview().map((meta) => meta.taskId)).toEqual([
      "fixture-task-allowed",
    ]);
  });
});
