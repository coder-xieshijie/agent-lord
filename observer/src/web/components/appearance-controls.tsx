...
function FontSelect({
  ...
  return (
    <label className="flex min-w-0 flex-col gap-1.5 text-xs text-muted-foreground">
      ...
      >
        <option value="system">
          系统默认{label === "代码字体" ? "等宽字体" : "字体"}
        </option>
        {value !== "system" && !families.includes(value) ? (
          <option value={value}>{value}（已保存）</option>
        ) : null}
        {families.length ? (
...
export function AppearanceControls() {
  ...
  const detect = async () => {
    setLoading(true);
    try {
      setCatalog(await fetchFonts());
    } catch {
      setCatalog({
        families: [],
        available: false,
        message: "字体列表暂不可用，可继续使用系统默认字体。",
      });
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <CollapsibleContent className="max-h-[45vh] overflow-y-auto border-t bg-card/60 px-4 py-4 sm:px-6">
        <div className="mx-auto grid max-w-4xl grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_5.5rem] gap-2">
            <FontSelect
              families={catalog?.families ?? []}
              label="代码字体"
              onChange={(codeFont) => change({ codeFont })}
              value={appearance.codeFont}
            />
            <FontSizeSelect
              label="代码字号"
              onChange={(codeFontSize) => change({ codeFontSize })}
              sizes={CODE_FONT_SIZES}
        ...
        <div className="mx-auto mt-3 flex max-w-4xl flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
          <span>
            {loading
              ? "正在检测本机字体…"
              : catalog?.available
                ? `已识别 ${catalog.families.length} 种本机字体 · 缺少的字形使用系统回退字体`
                : catalog?.message}
          </span>
        ...
        <div className="mx-auto mt-3 flex max-w-4xl flex-wrap gap-x-5 gap-y-1 border-t pt-3 text-sm">
          ...
          <code className="observer-code font-mono text-xs text-muted-foreground">
