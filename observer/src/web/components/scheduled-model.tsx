...
export function ScheduledModel({
  meta,
}: {
  meta: Pick<TaskMeta, "provider" | "model" | "effort" | "execution">;
}) {
  const { name, full, strength, strengthSource } =
    summarizeScheduledModel(meta);
  if (!name) return null;
  const strengthTitle =
    strengthSource === "variant"
      ? "请求推理档位（MCode variant）"
      : "请求 Effort";
  return (
    <span className="flex min-w-0 items-center gap-1.5">
      <span
        className="max-w-40 truncate font-mono text-muted-foreground text-xs"
        title={full ?? name}
      >
        {name}
      </span>
