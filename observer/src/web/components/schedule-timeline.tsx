...
function OperationDetail({
  ...
  const foreignCaller =
    op.callerSessionId &&
    group.sessionId &&
    op.callerSessionId !== group.sessionId;
  const rows: Array<[string, string]> = [
    ["派发（操作创建）", fmt(op.createdAtMs)],
    [
      "执行端开始",
      op.executorStartedAtMs === null
        ? "无 journal 证据"
        : fmt(op.executorStartedAtMs),
    ],
    ...
    [
      "交付核验",
      op.deliveryStatus === "verified"
        ? "文件/提交检查通过（无独立时间戳）"
        : op.deliveryStatus === "incomplete"
          ? "交付项未齐"
          : op.deliveryStatus === "unverified"
            ? "未核验"
            : "未声明",
    ],
  ];
  return (
    <div className="mt-2 space-y-1 rounded-md bg-muted/40 p-3 text-xs">
      <p className="break-all font-medium">
        {task?.title ?? taskId} · 操作 {op.operationId}
      </p>
      <p className="text-muted-foreground">
        触发：{op.trigger} · 派发 Turn：{op.dispatchTurnId ?? "未绑定"}
        {op.dispatchBinding === "inferred-by-create-time"
          ? "（按创建时间推断）"
          : op.dispatchBinding === "recorded"
            ? "（记录）"
            : ""}
      ...
      {foreignCaller ? (
        <p className="text-amber-600 dark:text-amber-400">
          本轮实际调用者是其他会话：{op.callerSessionId}
          （归属仍按最初拉起的调度会话）
        </p>
      ) : null}
      {op.clockAnomaly ? (
        <p className="text-destructive">
          时间顺序异常：记录的结束时间早于创建时间，按原始证据展示
        </p>
      ) : null}
      {task?.parallel ? (
        <p className="text-muted-foreground">
          并行组 {task.parallel.group} ·{" "}
          {task.parallel.role === "integrator"
            ? `integrator，等待全部 workers：${task.parallel.workers.join("、")}`
            : `worker → integrator 任务 ${task.parallel.integratorTaskId}`}
        </p>
      ) : null}
      {rows.map(([name, value]) => (
        <p className="break-all" key={name}>
          <span className="text-muted-foreground">{name}：</span>
          {value}
        </p>
      ))}
      {op.errorCode ? (
        <p className="text-destructive">错误码：{op.errorCode}</p>
      ) : null}
      <p className="text-muted-foreground">
        收到回执仅表示调度会话已接收结构化结果，不代表已分析；主调度 Turn
        结束是另一独立事实。
      </p>
    </div>
  );
...
export function ScheduleTimelinePanel({
        ...
        setGroup(
          schedule.groups.find(
            (item) =>
              ...
              (item.kind ?? "codex") === (callerKind ?? "codex"),
          ) ?? null,
        );
        setNow(schedule.now);
        setError(null);
  ...
  const options = useMemo(
    () => (group ? rangeOptions(group, now) : []),
    [group, now],
  );
  const activeKey = options.some((option) => option.key === rangeKey)
    ? rangeKey
    : defaultRangeKey(options);
  ...
  const edges = useMemo(
    () => (layout ? edgesForSelection(layout, selection) : []),
    [layout, selection],
  );
  const selectedOp = useMemo(() => {
    if (!group || !selection) return null;
    const op = group.tasks
      .find((task) => task.taskId === selection.taskId)
  ...
  return (
    <section
      aria-label="调度时间线"
      className="max-h-[55vh] shrink-0 overflow-y-auto border-b px-4 py-3 sm:px-6"
    >
      ...
      {group && group.turnsNote ? (
        <p className="mt-2 text-muted-foreground text-xs">
          调度记录说明：{group.turnsNote}
        </p>
      ) : null}
      {!group && !error ? (
        <p className="mt-2 text-muted-foreground text-xs">正在加载调度聚合…</p>
      ) : null}
      {group && !layout ? (
        <p className="mt-2 text-muted-foreground text-xs">
          该分组还没有任何带时间证据的操作。
      ...
      {layout && range ? (
        <div className="mt-3 flex gap-2">
          ...
          <div className="w-40 shrink-0 space-y-1">
            <div
              className="flex h-8 items-center truncate font-medium text-xs"
              style={{ height: ROW_H }}
            >
              主调度会话
            </div>
          ...
          >
            {/* Main scheduler lane */}
            <div
              className="absolute inset-x-0 rounded-md bg-muted/30"
              ...
              {layout.turnSegments.map((segment) => (
                <div
                  ...
                  style={{
                    ...
                    width: `${Math.max((segment.endFrac - segment.startFrac) * 100, 0.4)}%`,
                  }}
                ...
                >
                  {segment.clippedStart ? (
                    <span className="absolute top-0 left-0 text-[9px] text-muted-foreground">
                      …
                    </span>
                  ) : null}
                  {segment.clippedEnd ? (
                    <span className="absolute top-0 right-0 text-[9px] text-muted-foreground">
                      …
                    </span>
                  ) : null}
                </div>
            ...
            {layout.lanes.map((lane, laneIndex) => (
              <div
                className="absolute inset-x-0 rounded-md bg-muted/20"
                key={lane.task.taskId}
                style={{
                  top: (laneIndex + 1) * (ROW_H + ROW_GAP),
                  height: ROW_H,
                }}
              >
                {lane.segments.map((segment) => {
                  const selected =
                    selection?.taskId === segment.taskId &&
                    selection.operationId === segment.operationId;
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        segment.op.clockAnomaly &&
                          "outline-1 outline-dashed outline-destructive",
                      )}
                      ...
                      style={{
                        left: `${segment.startFrac * 100}%`,
                        width: `${Math.max((segment.endFrac - segment.startFrac) * 100, 0.6)}%`,
                      }}
                ...
                {lane.segments.flatMap((segment) =>
                  segment.markers
                    .filter((marker) => marker.kind !== "dispatch")
                    .map((marker) => (
                      <span
                        className={cn(
                          "pointer-events-none absolute top-0.5 h-1.5 w-0.5 rounded-full",
                          MARKER_STYLE[marker.kind].className,
                        )}
              ...
              >
                {edges.map((edge) => (
                  <line
                    ...
                    stroke={
                      edge.kind === "dispatch" ? "currentColor" : "#f59e0b"
                    }
      ...
      {layout ? (
        <p className="mt-1 text-[10px] text-muted-foreground">
          标记：
          <span className="text-sky-600 dark:text-sky-400">执行端开始</span> ·
          <span> 执行结束</span> ·{" "}
          <span className="text-violet-600 dark:text-violet-400">产物可用</span>{" "}
          ·
          <span className="text-amber-600 dark:text-amber-400">
            {" "}
            调度收到回执
          </span>
          ；缺失的时间点不显示，虚线段表示尚未观测到结束。点击执行段查看关联与详情。
        </p>
      ) : null}
      {group && selection && selectedOp ? (
        <OperationDetail
          group={group}
          op={selectedOp}
