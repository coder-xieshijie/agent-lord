...
export function requestLabel(item: RequestItem): string {
  ...
  if (item.trigger === "caller_followup")
    return `${item.caller?.kind === "codex" ? "Codex" : "调度方"} 补充请求`;
  return "实际派发请求";
}
...
export function RequestRow({
  ...
  return (
    <section
      className="my-3 min-w-0 rounded-lg border bg-muted/30 p-3"
      aria-label={requestLabel(item)}
      data-opid={item.opId}
    >
      ...
      {item.role === "caller" && (
        <p className="mt-1 break-all text-xs text-muted-foreground">
          调度 Session：{item.caller?.session_id ?? "未记录"}
        </p>
      )}
      {item.reason && (
      ...
      {item.role === "caller" && !item.originalRecorded && (
        <p className="mt-1 text-xs text-muted-foreground">
          用户原始请求未单独记录；以下为实际派发内容。
        </p>
      )}
      {long ? (
        <details
        ...
        >
          <summary className="cursor-pointer text-sm">
            {open ? "收起" : "展开"}请求全文（{item.text.length} 字符）
          </summary>
