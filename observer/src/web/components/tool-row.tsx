...
function CodeDetail({
  ...
  return (
    <CodeBlock
      className="tool-code text-xs [&_pre]:max-h-80 [&_pre]:text-xs"
      code={code}
      language={language}
...
function ToolDetails({ item }: { item: ToolItem }) {
  ...
  try {
    if (item.inputText) input = JSON.parse(item.inputText);
  } catch {
    /* text input */
  }
  const record =
  ...
  const extraInput =
    record &&
    Object.keys(record).some((key) => key !== "command" && key !== "cmd");
  const file =
  ...
  return (
    <div className="space-y-3">
      ...
      {command ? (
        <CodeDetail code={command} label="命令" language="shellscript" />
      ) : null}
      {item.inputText && (!command || extraInput) ? (
        <CodeDetail
          code={
            typeof input === "string" ? input : JSON.stringify(input, null, 2)
          }
          label="参数"
          language={typeof input === "string" ? "shellscript" : "json"}
        />
      ) : null}
      {item.outputText !== undefined || item.state === "running" ? (
        ...
        >
          <TerminalHeader className="border-border px-3 py-1.5">
            <TerminalTitle className="text-xs text-muted-foreground">
              输出
            </TerminalTitle>
            <TerminalActions>
              {item.state === "running" ? (
          ...
          <TerminalContent
            className="max-h-80 p-3 text-xs"
            onScroll={(event) => {
      ...
      {item.errorText ? (
        <pre
          className={cn(
            "max-h-64 overflow-auto whitespace-pre-wrap break-words rounded-md p-3 font-mono text-xs",
            item.state === "warning"
      ...
      {item.exitCode !== undefined ? (
        <p
          className={cn(
            ...
            item.exitCode !== 0 && "text-destructive",
          )}
        >
          退出码 {item.exitCode}
        </p>
      ...
      {!item.inputText &&
      item.outputText === undefined &&
      !item.errorText &&
      item.state !== "running" ? (
        <p className="text-xs text-muted-foreground">此工具未提供更多详情。</p>
      ) : null}
    </div>
  );
}
...
export function ToolRow({
    ...
    item.state === "running"
      ...
      : item.state === "error"
        ...
        : item.state === "warning"
          ...
          : "已完成";
  const timing = [
    item.preparationMs !== undefined
      ? `参数准备 ${(item.preparationMs / 1000).toFixed(2)}s`
      : null,
    item.executionMs !== undefined
      ? `执行 ${(item.executionMs / 1000).toFixed(2)}s`
      : null,
  ]
    .filter(Boolean)
    .join(" · ");
  return (
    <Tool
      className="tool-row mb-0 rounded-none border-0"
      onOpenChange={onOpenChange}
      open={open}
    >
      <CollapsibleTrigger
        aria-label={`${item.name} · ${status}`}
        className="group flex min-h-8 w-full items-center gap-2 rounded-md px-1.5 py-1 text-left text-xs text-muted-foreground hover:bg-accent/60 hover:text-foreground"
      >
        ...
        {timing ? (
          <span className="ml-auto shrink-0 text-[0.6875rem]">{timing}</span>
        ) : null}
        {item.state === "error" ? (
          <span className="shrink-0 text-destructive">执行失败</span>
        ) : null}
        {item.state === "warning" ? (
...
export function CompletedToolGroup({
  group,
  expanded,
  setExpanded,
}: {
  ...
  const open =
    expanded[group.id] ?? group.tools.some((tool) => expanded[tool.id]);
  return (
    <Collapsible
      onOpenChange={(value) => setExpanded(group.id, value)}
      open={open}
    >
