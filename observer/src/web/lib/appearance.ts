...
export const CODE_FONT_SIZES = Array.from(
  { length: 15 },
  (_, index) => index + 10,
);
...
const validFont = (value: unknown): value is string =>
  typeof value === "string" &&
  value.length > 0 &&
  value.length <= 200 &&
...
export function readAppearance(storage: Pick<Storage, "getItem">): Appearance {
  try {
    ...
    const value: Partial<Appearance> | null = saved
      ? (JSON.parse(saved) as Partial<Appearance> | null)
    ...
    return {
      ...
      codeFont: validFont(value?.codeFont)
        ? value.codeFont
        : DEFAULT_APPEARANCE.codeFont,
      fontSize: UI_FONT_SIZES.includes(value?.fontSize ?? NaN)
        ? value!.fontSize!
        : DEFAULT_APPEARANCE.fontSize,
      codeFontSize: CODE_FONT_SIZES.includes(value?.codeFontSize ?? NaN)
        ? value!.codeFontSize!
...
export function applyAppearance(value: Appearance, systemDark: boolean): void {
  ...
  root.style.setProperty(
    "--observer-code-font-size",
    `${value.codeFontSize}px`,
  );
}
