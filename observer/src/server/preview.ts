...
export async function stopPreview(
  ...
  if (expectedInstance && record.instance_id !== expectedInstance)
    throw new Error("observer 实例已变化；未停止新实例，请重试绑定");
  if (!(await probe(record)))
    throw new Error(
      "无法确认该端口和 PID 属于记录的 observer 实例；未发送停止信号",
    );
...
export async function startPreview(
  ...
  const entrypoint =
    options.entrypoint ?? fileURLToPath(new URL("./main.js", import.meta.url));
  const existing = readMetadata(options.stateDir, options.port);
  if (existing && (await probe(existing))) {
    if (
      JSON.stringify(existing.tasks) !== JSON.stringify(options.tasks) ||
      existing.web_root !== options.webRoot ||
      existing.refresh_ms !== options.refreshMs ||
      existing.entrypoint !== entrypoint ||
      JSON.stringify(existing.run_ids ?? []) !==
        JSON.stringify(options.runIds ?? []) ||
      (options.token && previewToken(existing) !== options.token)
    ) {
      throw new Error(
        "该端口的 observer 配置不同；请显式使用 preview:restart 应用新配置",
      );
    }
    return existing;
  }
  if (existing && pidAlive(existing.pid))
    throw new Error(
      "预览记录的 PID 仍存在但实例身份无法确认；未启动或终止任何进程",
    );
  const token =
    options.token ||
    (existing ? previewToken(existing) : randomBytes(16).toString("base64url"));
  ...
  while (Date.now() < deadline) {
    ...
    if (record && record.pid === child.pid && (await probe(record)))
      return record;
    await delay(100);
  }
  if (child.exitCode === null && child.signalCode === null)
    child.kill("SIGTERM");
  throw new Error(
    `预览未就绪${spawnError ? `：${String(spawnError)}` : ""}；查看 ${logPath}`,
  );
}
...
export async function attachPreview(options: LaunchOptions) {
  ...
  if (!options.tasks.includes(focus))
    throw new Error("focus-task 必须属于本次 --tasks");
  let record = readMetadata(options.stateDir, options.port);
  if (record) {
    if (!(await probe(record)))
      throw new Error("无法核验已有 observer；未停止或替换服务");
    const tasks = [...new Set([...record.tasks, ...options.tasks])].sort();
    const runIds = [
      ...new Set([...(record.run_ids ?? []), ...(options.runIds ?? [])]),
    ].sort();
    if (
      tasks.some((task) => !record!.tasks.includes(task)) ||
      runIds.some((id) => !record!.run_ids?.includes(id))
    ) {
      const old = record;
      ...
      record = await startPreview({
        ...
        tasks,
        runIds,
        token: previewToken(old),
        webRoot: old.web_root,
        refreshMs: old.refresh_ms,
  ...
  const json = async (route: string) => {
    ...
    if (
      ...
      throw new Error(`绑定 HTTP 核验失败：${route}`);
    return response.json() as Promise<Record<string, unknown>>;
  };
  if (options.runIds?.length) {
    const binding = await json("/api/binding");
    if (
      binding.instanceId !== record.instance_id ||
      binding.pid !== record.pid ||
      !Array.isArray(binding.run_ids) ||
      options.runIds.some(
        (id) => !(binding.run_ids as unknown[]).includes(id),
      ) ||
      !binding.errors ||
      typeof binding.errors !== "object" ||
      Object.keys(binding.errors).length
    )
      throw new Error("run 自动绑定核验失败");
  }
  const overview = await json("/api/overview");
  const tasks = Array.isArray(overview.tasks)
    ? (overview.tasks as Array<{ taskId?: string }>)
    : [];
  if (
    options.tasks.some((task) => !tasks.some((entry) => entry.taskId === task))
  )
    throw new Error("overview 缺少本次任务");
  const snapshots = await Promise.all(
    options.tasks.map(async (task) => {
      const snapshot = await json(
        `/api/tasks/${encodeURIComponent(task)}/snapshot`,
      );
      const meta = snapshot.task as
        | { taskId?: string; available?: boolean; status?: string }
        | undefined;
...
async function main(): Promise<void> {
  ...
  if (!["start", "status", "stop", "restart", "attach"].includes(action))
    throw new Error(
      "用法：preview start|status|stop|restart|attach [--tasks id,...] [--focus-task id] [--port 8791]",
    );
  ...
  if (action === "attach") {
    ...
    console.log(
      JSON.stringify(await previewStatus(options.stateDir, options.port)),
    );
  } else if (action === "stop") {
    ...
    console.log(
      JSON.stringify({ status: "running", ...(await startPreview(options)) }),
    );
  }
}
