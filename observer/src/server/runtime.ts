...
export function readMetadata(root: string, port: number): PreviewRecord | null {
  ...
  if (
    ...
    value.pid <= 0 ||
    value.port !== port ||
    value.state_dir !== path.resolve(root) ||
    !Array.isArray(value.tasks) ||
    !value.web_root ||
    !value.url
  ) {
    throw new Error(
      "预览记录缺少可核验的实例身份；请先检查旧服务的进程与端口归属",
    );
  }
  return value;
}
...
export function previewToken(record: PreviewRecord): string {
  ...
  if (url.origin !== `http://127.0.0.1:${record.port}` || !token)
    throw new Error("非法预览地址");
  return token;
}
...
export async function probe(record: PreviewRecord): Promise<boolean> {
  try {
    ...
    return (
      health.service === "agent-lord-observer" &&
