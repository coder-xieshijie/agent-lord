...
const object = (v: unknown): Record<string, unknown> =>
  v !== null && typeof v === "object" && !Array.isArray(v)
    ...
    : {};
const unknown = (note: string): CallerLifecycle => ({
  status: "unknown",
  turnId: null,
  startedAtMs: null,
  completedAtMs: null,
  ...
  const visit = (dir: string, depth: number): void => {
    ...
    for (const file of files) {
      ...
      if (file.isDirectory()) visit(path.join(dir, file.name), depth + 1);
        ...
        file.name.endsWith(`-${session}.jsonl`)
      )
...
export class CallerLifecycleReader {
  ...
  private pump(
  ...
  ): { reader: Reader; size: number; mtimeMs: number } | { note: string } {
    if (
      caller?.kind !== "codex" ||
      !caller.session_id ||
      !caller.data_root ||
      ...
      return { note: "未记录可核验的 Codex 调度会话来源" };
    ...
    try {
      ...
      if (
        (reader.inode && reader.inode !== stat.ino) ||
        stat.size < reader.offset
      ) {
        reader.offset = 0;
        reader.verified = false;
        reader.rejected = false;
        reader.active = null;
        reader.turns.clear();
      }
      ...
      for (const line of tail.lines) {
        ...
        if (event.type === "session_meta") {
          if (payload.id !== caller.session_id) {
            reader.rejected = true;
            reader.verified = false;
            reader.turns.clear();
        ...
        if (event.type === "event_msg") {
          if (
            ...
            if (!reader.turns.has(payload.turn_id))
              reader.turns.set(payload.turn_id, {
                id: payload.turn_id,
                started: ts,
                completed: null,
                aborted: false,
            ...
            if (reader.turns.size > 1000)
              reader.turns.delete(reader.turns.keys().next().value!);
          } else if (
            ...
            if (turn && ts >= turn.started) {
              turn.completed = ts;
              turn.aborted = payload.type === "task_aborted";
            }
      ...
      if (!reader.verified || reader.rejected)
        return { note: "Codex 日志的 Session 身份不匹配" };
      if (reader.offset < stat.size)
        return { note: "正在读取调度生命周期记录" };
  ...
  sessionTimeline(caller: CallerIdentity | undefined): SessionTimeline {
    if (caller?.kind === "mcode" || caller?.kind === "claude")
      return this.native.read(caller).timeline;
    const pumped = this.pump(caller);
    if ("note" in pumped)
      return { turns: [], receipts: new Map(), note: pumped.note };
    const turns = [...pumped.reader.turns.values()].sort(
    ...
    return {
      turns: turns.map((turn) => ({
        turnId: turn.id,
        startedAtMs: turn.started,
        completedAtMs: turn.completed,
  ...
  observe(
    ...
    if (caller?.kind === "mcode" || caller?.kind === "claude") {
      ...
      const turn = caller.turn_id
        ? ordered.find((turn) => turn.turnId === caller.turn_id)
        : ordered.filter((turn) => turn.startedAtMs <= created).at(-1);
      if (
        !turn ||
        !Number.isFinite(created) ||
        created < turn.startedAtMs ||
        (turn.completedAtMs !== null && created > turn.completedAtMs)
      )
        return unknown(
          snapshot.timeline.note || "无法将本次操作绑定到调度 Turn",
        );
      const receipt = snapshot.timeline.receipts.get(opId);
      const received =
        receipt?.status === "SUCCEEDED" &&
        receipt.turnId === turn.turnId &&
        (readyAt === null || receipt.atMs >= readyAt)
          ? receipt.atMs
          : null;
      return {
        status:
          turn.completedAtMs !== null
            ? turn.aborted
              ? "aborted"
              : "completed"
            : ordered.some((other) => other.startedAtMs > turn.startedAtMs) ||
                !snapshot.lifecycleKnown
              ? "unknown"
              : "running",
        turnId: turn.turnId,
    ...
    const created = Date.parse(createdAt);
    const ordered = [...reader.turns.values()].sort(
      (a, b) => a.started - b.started,
    );
    const turn = caller?.turn_id
      ? reader.turns.get(caller.turn_id)
      : ordered.filter((item) => item.started <= created).at(-1);
    if (
      !turn ||
      !Number.isFinite(created) ||
      created < turn.started ||
      (turn.completed !== null && created > turn.completed)
    )
      return unknown("无法将本次操作绑定到调度 Turn");
    const hasLaterTurn = ordered.some((item) => item.started > turn.started);
    const receipt = turn.receipts.get(opId) ?? null;
    const received =
      receipt && receipt.status === "SUCCEEDED" ? receipt.atMs : null;
    return {
      status:
        turn.completed !== null
          ? turn.aborted
      ...
      completedAtMs: turn.completed,
      receivedAtMs:
        received !== null && (readyAt === null || received >= readyAt)
          ? received
          : null,
