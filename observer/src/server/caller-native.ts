...
const object = (v: unknown): Record<string, unknown> =>
  v && typeof v === "object" && !Array.isArray(v)
    ...
    : {};
const title = (v: unknown): string | null =>
  typeof v === "string" ? clipTitle(v) || null : null;
const project = (v: unknown): string | null =>
  typeof v === "string" ? title(path.basename(v)) : null;
const empty = (note: string): Snapshot => ({
  name: null,
  projectName: null,
  observedAtMs: null,
  lifecycleKnown: false,
  timeline: { turns: [], receipts: new Map(), note },
});
...
export class NativeCallerReader {
  ...
  read(caller: CallerIdentity): Snapshot {
    ...
    try {
      value =
        caller.kind === "mcode"
          ? this.mcode(caller)
          : this.readClaude(caller, key);
    } catch {
      value = empty("调度会话记录暂不可读或格式不支持");
    }
  ...
  private mcode(caller: CallerIdentity): Snapshot {
    ...
    try {
      ...
      const session = db
        .prepare(
          "SELECT title, workspace_dir FROM local_runtime_sessions WHERE session_id = ?",
        )
        .get(caller.session_id!);
      if (!session) return empty("MCode 数据根中没有匹配的 Session");
      const result = empty("");
      result.name = title(session.title);
      result.projectName = project(session.workspace_dir);
      result.observedAtMs = Date.now();
      result.lifecycleKnown = true;
      const rows = db
        .prepare(
          "SELECT turn_id, status, accepted_at_ms, completed_at_ms FROM local_runtime_turn_ingress WHERE session_id = ? ORDER BY accepted_at_ms DESC LIMIT ?",
        )
        .all(caller.session_id!, MAX_TURNS + 1);
      if (rows.length > MAX_TURNS)
        result.timeline.note = "仅展示最近 1000 个调度回合";
      result.timeline.turns = rows
        .slice(0, MAX_TURNS)
        .reverse()
        .flatMap((row) => {
          ...
          return [
            {
              ...
              startedAtMs: started,
              completedAtMs:
                row.status !== "accepted" && ended !== null && ended >= started
                  ? ended
                  : null,
      ...
      result.timeline.note ||=
        "MCode 回合状态来自持久化 ingress；工具回执时间未记录";
      return result;
    } finally {
      db.close();
    }
  ...
  private receive(
    ...
    turnId: string,
    ts: number,
    index: Map<string, ScheduleReceipt>,
  ): void {
    const found = {
      receipts: new Map<
        string,
        { atMs: number; status: ScheduleReceipt["status"] }
      >(),
    };
    collectReceipts(value, found, ts);
    for (const [id, receipt] of found.receipts)
      if (!index.has(id)) index.set(id, { ...receipt, turnId });
  ...
  private readClaude(caller: CallerIdentity, key: string): Snapshot {
    ...
    const file =
      state?.file ?? locateClaudeSession(caller.data_root!, caller.session_id!);
    ...
    try {
      ...
      return empty("Claude Code 日志暂不可读");
    }
    ...
    for (const line of tail.lines) {
      ...
      if (event.sessionId !== caller.session_id || event.isSidechain === true)
        continue;
      state.verified = true;
      if (event.type === "custom-title")
        state.name = title(event.customTitle) ?? state.name;
      ...
      const toolResults = Array.isArray(content)
        ? content.filter((item) => object(item).type === "tool_result")
        : [];
      if (event.type === "user" && toolResults.length) {
        ...
        const text =
          typeof content === "string"
            ...
            : Array.isArray(content)
              ? content
                  .map((item) =>
                    object(item).type === "text" ? object(item).text : "",
                  )
        ...
        state.active &&
        ts >= state.active.startedAtMs
      ) {
        state.active.completedAtMs = ts;
        state.active = null;
        state.pendingStop = false;
      }
    }
      ...
      timeline: {
        ...
        note:
          state.offset < stat.size
            ? "正在读取 Claude Code 调度日志"
            : state.pendingStop
              ? "已观测模型回答结束，尚无 Claude Code 回合结束事件"
              : "",
      },
    };
  }
...
function locateClaudeSession(root: string, session: string): string | null {
  ...
  try {
    ...
    for (const dir of readdirSync(projects, { withFileTypes: true })) {
      ...
      try {
        if (lstatSync(file).isFile()) found.push(file);
      } catch {
        /* no matching session here */
      }
    }
  } catch {
    return null;
