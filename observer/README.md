...
Details fully show the requested model, the actual model, the reasoning effort, and the verification source. Reasoning effort is an independent, argument-enforced launch setting, not part of the model name; a `#variant` suffix belongs to the model identity, and the schema-v1 terminal report does not echo effort back. When Codex has only argument-enforced evidence, the actual model still shows as unreported. Claude fallback shows the model that actually ran. The initial scheduler and this turn's caller come from the first and the current operation.invocation respectively, marked unrecorded when missing.
