...
5. Start a later round with `turn` only after the previous operation is terminal. The script reapplies the saved execution contract and deduplicates an identical in-flight message. For concrete local CLI deliverables, declare each non-empty workspace-relative file with `--require-file` and a new clean commit with `--require-commit` on `start` or `turn`. These structural checks do not establish test results, UI quality, or semantic acceptance.
...
The observer is a TypeScript/Node service with React and vendored Vercel AI Elements components. Build with `cd observer && pnpm install --frozen-lockfile && pnpm build`, then run `pnpm preview:start --tasks <id[,id…]> --port 8791`. Use `preview:status`, `preview:restart`, and `preview:stop` with the same port/state directory. Restart retains the token and address; the launcher checks authenticated instance identity and PID before stopping a service. Open the returned URL with Codex `open_in_codex` (browser target) or in an ordinary browser. Never infer readiness from a background shell's exit code: require the launcher's verified `running` result.
