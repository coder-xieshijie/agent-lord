...
4. In Codex Desktop, after dispatch and before the first supervision wait, follow [observer task attachment and display verification](observer/README.md#派发后的任务绑定) for this task or the explicitly requested pipeline task set. Apply this step when starting, continuing, or recovering work; skip automatic observer setup only when the user explicitly requests background execution without a page. If dispatch yields a running host-command handle, attach the observer while that controller runs and retain the handle for later resumption.
5. Process the returned envelope until terminal:
   ...
   - `SUCCEEDED`: use the returned artifact as the canonical response. This is execution success; inspect `delivery` separately before claiming the requested files or commit are complete.
...
6. Start a later round with `turn` only after the previous operation is terminal, then repeat the observer step before supervision. The script reapplies the saved execution contract and deduplicates an identical in-flight message. For concrete local CLI deliverables, declare each non-empty workspace-relative file with `--require-file` and a new clean commit with `--require-commit` on `start` or `turn`. These structural checks do not establish test results, UI quality, or semantic acceptance.
...
The observer is a TypeScript/Node service with React and vendored Vercel AI Elements components. Build with `cd observer && pnpm install --frozen-lockfile && pnpm build` when its dependencies or compiled assets are missing or stale. The caller owns task attachment and browser display through the existing preview commands and host tools; [observer/README.md](observer/README.md#派发后的任务绑定) defines reuse, task-list updates, and completion checks. The launcher checks authenticated instance identity and PID before stopping a service.
