...
- Use CC Fable as the default only at a genuine judgement or decision node when the user specified neither its decision model nor that the decision should return to them. Give it enough context to decide; never inject Fable as a fixed workflow node or restrict other Claude Code endpoints from ordinary execution.
- `claude-cli`: default to `claude-opus-5` with `high`. A model-only request still uses `high`. The default retry plan permits five attempts. When the primary model belongs to the Fable family, five failed attempts are followed by up to five `claude-opus-5` attempts on the same session; only exhaustion of both stages is terminal.
