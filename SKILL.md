...
`python3 scripts/observer_server.py --task <task_id> [--task ...] [--port N]` serves a loopback-only web view (snapshot + monotonic cursor + SSE) of the allow-listed tasks: control-plane journal events plus a sanitized projection of the native exec streams for `codex-cli`, `claude-cli`, and `mcode-cli` (`codex-app` shows state only, no stream). It is strictly read-only against dispatch state — it never triggers checks, checkpoints, recovery, or prompts — and its own runtime files live under the independent `<state>/observer/` namespace. Every request requires the startup access token printed as a JSON line (`url`, `pid`, `port`); reasoning traces, raw logs, prompts, and credentials are never served. The task view offers a copyable native resume command (`claude --resume ID`, `codex resume -C WORKTREE ID`, `mcode --session ID`) only when a session id is journaled, and marks running tasks as not attachable. Stopping the server never affects the observed tasks.
